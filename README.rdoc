...
Here is what each format token means:
  ...
  Special Cases:
    ...
    mmmm = month long name in any available locale (e.g. matches 'Ene' even with I18n.locale == :en)
...
<em>This section applies to +mmmm+ token exclusively.</em>

If you have strings with non-standart (e.g. pluralized) month names, they won't match against names stored in I18n. If you don't want to fire up a full-fledged pluralization (and you don't), you can add names you need on initialization (or later)

  Timeliness.month_names [[nil, 'Января', 'Февраля', ...]]

Format is identical to <tt>Date::ABBR_MONTHNAMES</tt>.
